...
Crush's file writing tools (`write`, `edit`, `multiedit`) all operate on the full file content. The `write` tool overwrites a file entirely, while `edit` and `multiedit` require reading the file first to perform find-and-replace operations.
...
**Why This Was Rejected:**
- Shell-based appending bypasses Crush's permission system, file history tracking, and LSP notifications.
