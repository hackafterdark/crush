...
Crush's AI agents need to locate code by syntax structure (functions, structs, calls, imports) rather than plain text search. The existing `grep` tool works for literal text patterns but cannot understand code structure, handle AST-aware queries, or navigate complex nested syntax.
...
4. Integrates seamlessly with Crush's agent tool funnel
