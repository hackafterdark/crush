...
Crush allows you to customize all TUI key bindings through `crush.json`. This is useful when your OS or terminal intercepts certain key combinations (e.g., `ctrl+alt+v` for clipboard on Linux, `ctrl+v` on Windows terminals, `ctrl+alt+<key>` on macOS).

## Configuration

Add keybindings under `options.tui.keybindings` in your `crush.json`:
...
Use the built-in `crush bindings` command to list all available keybindings grouped by section:
...
This feature is fully backward-compatible. Existing `crush.json` files without `keybindings` will continue to work with all defaults unchanged.
...
- Key bindings are read at startup. Changes to `crush.json` require a restart of Crush.
