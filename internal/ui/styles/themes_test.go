...
func TestThemeBuiltin(t *testing.T) {
	...
	stylesObsidiana := HypercrushObsidiana()
	if hex(stylesNamedObsidiana.Background) == nil || hex(stylesObsidiana.Background) == nil ||
		*hex(stylesNamedObsidiana.Background) != *hex(stylesObsidiana.Background) {
		t.Errorf("expected 'obsidiana' named theme to match Hypercrush Obsidiana")
	}
}
...
func TestThemeLookup(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(wsDir, ".crush", "themes"), 0o755); err != nil {
...
`
	...
	// 3. Local .crush/themes/ theme.
	dotCrushThemeContent := `
name: Dot Crush Theme
primary: "#000003"
`
	dotCrushThemePath := filepath.Join(wsDir, ".crush", "themes", "mytheme.yaml")
	if err := os.WriteFile(dotCrushThemePath, []byte(dotCrushThemeContent), 0o644); err != nil {
		t.Fatalf("failed to write dot crush theme: %v", err)
	}

	// 1. Should load from .crush/themes first (highest priority).
	...
	// 2. Remove .crush/themes theme, should load from themes/ (medium priority).
	if err := os.Remove(dotCrushThemePath); err != nil {
