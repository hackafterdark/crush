...
func globalThemesDir() string {
	if crushGlobal := os.Getenv("CRUSH_GLOBAL_CONFIG"); crushGlobal != "" {
		return filepath.Join(crushGlobal, "themes")
	}
...
func findThemePath(themeOption string, workingDir string) string {
	...
	if workingDir != "" {
		localCrushThemesDir := filepath.Join(workingDir, ".crush", "themes")
		for _, ext := range extensions {
			candidates = append(candidates, filepath.Join(localCrushThemesDir, themeOption+ext))
		}
	...
	if workingDir != "" {
		candidates = append(candidates, filepath.Join(workingDir, ".crush", "themes", themeOption))
