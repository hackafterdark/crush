...
Crush supports paginated loading of chat message history to ensure optimal performance, low latency, and low memory usage—even during long sessions with hundreds or thousands of messages.
...
* **Startup Delay:** A long wait when entering or switching to a session as Crush processes the entire history.

To solve this, Crush loads session messages **lazily** using a sliding-window message pagination strategy.

---

## How It Works

1. **Initial Load:** When you switch to a session, Crush retrieves all message metadata lightweight but only instantiates the most recent `history_limit` messages as active UI components.
...
4. **Scroll & Selection Stability:** When new messages are prepended, Crush automatically calculates the offset and adjusts the viewport/selection index. The current scroll position is preserved seamlessly, preventing the screen from jumping.
...
You can customize the initial page size and subsequent batch size in your `crush.json` configuration file under the `options.tui` block:
