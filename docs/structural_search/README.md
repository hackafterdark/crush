...
You can customize query templates or add new ones by placing YAML files in `.crush/queries/` within your workspace directory. Crush watches this directory and reloads queries automatically on change.

Example (`.crush/queries/custom_rust.yaml`):
...
```powershell
$env:CGO_ENABLED="1"; $env:GOTOOLCHAIN="auto"; $env:PATH="F:/msys64/ucrt64/bin;" + $env:Path; go build -o crush.exe .
```
...
CGO_ENABLED=1 go build -o crush .

# Force a specific C compiler if needed (e.g. clang)
CC=clang CGO_ENABLED=1 go build -o crush .
```
...
CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go build -o crush-amd64 .
```
